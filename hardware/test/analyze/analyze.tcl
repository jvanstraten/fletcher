...
proc simtest {{unit_name "*"} {tc_name "*"}} {
  ...
  set testcases [glob $::env(FLETCHER_HARDWARE_DIR)/test/analyze/${unit_name}/${tc_name}_tc.vhd]
  set failures []
  set runs 0
  foreach testcase $testcases {
    ...
    foreach testbench [glob -nocomplain "$::env(FLETCHER_HARDWARE_DIR)/test/analyze/${unit_name}/*_tb.vhd"] {
      add_source $testbench
    }
    foreach test_vector [glob -nocomplain "$::env(FLETCHER_HARDWARE_DIR)/test/analyze/${unit_name}/*_tv.vhd"] {
      add_source $test_vector
    }
    add_source $::env(FLETCHER_HARDWARE_DIR)/test/analyze/${unit_name}/${tc_name}_tc.vhd
...
add_source $::env(FLETCHER_HARDWARE_DIR)/vhdl/axi/axi.vhd
add_source $::env(FLETCHER_HARDWARE_DIR)/vhdl/axi/axi_mmio.vhd
add_source $::env(FLETCHER_HARDWARE_DIR)/vhdl/axi/axi_tb_pkg.vhd
add_source $::env(FLETCHER_HARDWARE_DIR)/vhdl/axi/axi_tb_lite_master.vhd

add_source $::env(FLETCHER_HARDWARE_DIR)/vhdl/analyze/Analyze.vhd
add_source $::env(FLETCHER_HARDWARE_DIR)/vhdl/analyze/AnalyzeDistribution.vhd
...
if [batch_mode] {
  ...
  echo "Example: simtest AnalyzeDistribution"
}
